...
class Currency {
    ...
    private name: string;  //added type string
    private code: string;  //added type string
    private symbol: string; //added type string

    // Add types to the constructor parameters.

    constructor(name:string, code:string, symbol:string) {    //originally constructor(name, code, symbol)
        this.name = name;
        this.code=code;  
        this.symbol=symbol;
    ...
    describe() {
        ...
        let description = `The ${this.name} currency `;   //replaced null with this.name
